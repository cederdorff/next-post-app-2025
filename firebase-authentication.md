...
# Firebase Admin SDK (Server-side) - tilføjes senere
FIREBASE_PROJECT_ID=
...
## Del 2: Firebase Setup

### 2.1 Opret `lib/firebase.js`
...
### 3.1 Opret `contexts/AuthContext.js`
...
## Del 4: Login/Signup Sider

### 4.1 Opret `app/signin/page.js`
...
## Del 6: Navigation med Auth

### 6.1 Opdater `components/Nav.js`
...
## Del 7: Beskyt Server Actions

### 7.1 Opdater `app/posts/create/page.js`

**VIGTIGT:** Flyt Server Action til top-level og tilføj auth!
...
export default function CreatePage() {
  ...
  // Server Action nested i component (flytter vi ud)
  async function createPost(formData) {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        caption,
        image,
        uid: "OPPe5jue2Ghxx3mtnxevB5FwCYe2", // ← Hardcoded! Skal ændres
        createdAt: new Date().toISOString()
      })
    });
...
async function createPost(formData) {
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      caption,
      image,
      uid: user.uid, // ← Bruger authenticated user's UID
      createdAt: new Date().toISOString()
    })
  });
...
- ✅ Flyt `createPost` til top-level (uden for component)
- ✅ Tilføj auth check: `const user = await requireAuth().catch(() => redirect("/signin"))`
- ✅ Skift `uid: "hardcoded"` til `uid: user.uid`
- ✅ Flyt `url` ind i Server Action

### 7.2 Opdater `app/posts/[id]/update/page.js`
...
export default async function UpdatePage({ params }) {
  ...
  return (
    <section className={styles.formPage}>
      <div className={styles.container}>
        <h1>Update Post</h1>
        <FormPost action={updatePostWithId} post={post} />
      </div>
    </section>
  );
}
...
### 7.3 Opdater `app/posts/[id]/page.js`
      ...
      <div className={styles.container}>
        ...
        {isOwner && (
          <div className={styles.btns}>
            <DeletePostButton deleteAction={deletePostWithId} />
...
### 7.4 Tilføj styles til `app/posts/[id]/page.module.css`
...
## Del 8: Firebase Admin Credentials

### 8.1 Hent Service Account Key

1. Gå til [Firebase Console](https://console.firebase.google.com/)
2. Vælg dit projekt
3. Klik på **Project Settings** (tandhjul-ikonet)
4. Gå til fanen **Service Accounts**
5. Klik **Generate new private key**
6. En JSON-fil downloades

### 8.2 Tilføj credentials til `.env.local`

Fra den downloadede JSON-fil:
...
**VIGTIGT:** Tilføj `.env.local` til `.gitignore`!
